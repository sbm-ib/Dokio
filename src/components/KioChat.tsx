...
import { ArrowLeft, Loader2, FileText, AlertTriangle, Send, Copy, Check, Save, Download, Sparkles } from 'lucide-react'
...
export default function KioChat({ doc, onChangeDocument }: Props) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (code === 'limit_reached') {
        ...
        console.error('[KioChat] error:', err)
  ...
  const handleDownloadPdf = () => {
    try {
      ...
      console.error('[KioChat] pdf error:', err)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('[KioChat] save error:', err)
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm flex flex-col overflow-hidden">
      ...
      <div className="flex justify-between items-start p-6 pb-4 shrink-0">
        <div className="flex items-start gap-3">
          <button
            ...
            className="p-1.5 -ml-1.5 hover:bg-gray-100 rounded-lg transition-colors shrink-0 mt-0.5"
          ...
          <div>
            <h2 className="text-lg font-bold text-gray-900 flex items-center gap-1.5">
              <Sparkles size={16} className="text-paperliss" />
              Discute avec Kio
            </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 px-6 pb-6">
        {/* ── Chat ── */}
        <div className="flex flex-col h-[32rem] border border-gray-100 rounded-2xl overflow-hidden">
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {loadingHistory ? (
              ...
              <p className="text-sm text-gray-400 text-center py-8">
                Pose une question sur ce document, ou décris ce que tu veux répondre à Kio —
          ...
          <div className="border-t border-gray-100 p-3 flex items-end gap-2 shrink-0">
            <textarea
              ...
              placeholder="Écris ton message à Kio…"
              rows={2}
              disabled={sending}
        ...
        {/* ── Lettre ── */}
        <div className="flex flex-col h-[32rem] overflow-y-auto pr-1 space-y-4">
          {!lettre ? (
            <div className="flex-1 flex items-center justify-center text-center px-4">
              <p className="text-sm text-gray-400">
                La lettre apparaîtra ici dès que tu en demanderas une à Kio.
              </p>
            </div>
          ) : (
            <>
      ...
      {!canGenerateLetter() && (
        <p className="text-xs text-gray-400 text-center pb-4 px-6 shrink-0">
          {remainingCourriers() === 0
            ? 'Limite mensuelle de courriers générés atteinte — tu peux continuer à discuter avec Kio, mais pas régénérer la lettre.'
